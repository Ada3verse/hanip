...
export async function runFirebaseProductionRepositoryLocalTests() {
  ...
  check(document.schemaVersion === 2 && "settings" in document && "sessions" in document, "D document sections");
  ...
  check(fromFirestoreUserDocument({ ...document, schemaVersion: 99 }, uid).schemaVersion === 2, "I schema normalization");
  ...
  check((await imported.importUserData(uid, { schemaVersion: 3 })).error === "UNSUPPORTED_VERSION", "L future schema rejection");
