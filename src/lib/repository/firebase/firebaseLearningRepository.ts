...
export class FirebaseLearningRepository implements LearningRepository {
  ...
  async resetCurrentSession(userId: string) { const data = this.read(userId); if (data) await this.saveUserData(userId, { ...data, sessions: data.sessions.filter(({ sessionId }) => sessionId !== data.currentSessionId), currentSessionId: null, studentModel: null, goalState: null }); }
  async resetLearningProgress(userId: string) { const data = this.read(userId); if (data) await this.saveUserData(userId, { ...data, sessions: [], currentSessionId: null, studentModel: null, progress: createEmptyLearningProgress(), masteryProfiles: [], hintStates: [], workedExampleStates: [], misconceptionProfiles: [], adaptiveProfiles: [], goalState: null, sessionSummaries: [] }); }
  async exportUserData(userId: string): Promise<LearningDataExport> { return { schemaVersion: 1, exportedAt: new Date().toISOString(), data: this.read(userId) ?? createEmptyLearningUserData(userId) }; }
  async importUserData(userId: string, value: unknown): Promise<RepositoryImportResult> {
    ...
    if (typeof value.schemaVersion === "number" && value.schemaVersion > 1) return { success: false, error: "UNSUPPORTED_VERSION" };
    if (value.schemaVersion !== 1 || !record(value.data) || value.data.userId !== userId) return { success: false, error: "INVALID_DATA" };
