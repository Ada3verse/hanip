...
export class FirestoreLearningRepository implements LearningRepository {
  ...
  async resetCurrentSession(userId: string) { const data = await this.loadUserData(userId); if (data) await this.saveUserData(userId, { ...data, sessions: data.sessions.filter(({ sessionId }) => sessionId !== data.currentSessionId), currentSessionId: null, goalState: null }); }
  async resetLearningProgress(userId: string) { const data = await this.loadUserData(userId); if (data) await this.saveUserData(userId, { ...data, sessions: [], currentSessionId: null, studentModel: createEmptyRuntimeStudentModel(), progress: createEmptyLearningProgress(), masteryProfiles: [], hintStates: [], workedExampleStates: [], misconceptionProfiles: [], adaptiveProfiles: [], goalState: null, sessionSummaries: [] }); }
  async exportUserData(userId: string): Promise<LearningDataExport> { return { schemaVersion: 2, exportedAt: new Date().toISOString(), data: await this.loadUserData(userId) ?? createEmptyLearningUserData(userId) }; }
  async importUserData(userId: string, value: unknown): Promise<RepositoryImportResult> { if (!record(value)) return { success: false, error: "INVALID_DATA" }; if (typeof value.schemaVersion === "number" && value.schemaVersion > 2) return { success: false, error: "UNSUPPORTED_VERSION" }; if (![1, 2].includes(Number(value.schemaVersion)) || !record(value.data) || value.data.userId !== userId) return { success: false, error: "INVALID_DATA" }; try { const data = normalizeLearningUserData(value.data, userId); await this.saveUserData(userId, data); return { success: true, data }; } catch { return { success: false, error: "SAVE_FAILED" }; } }
